...
class ProviderProfileController extends Controller
{
    ...
    public function updateImage(Request $request)
    {
        ...
        $request->validate([
            ...
            'profile_image.required' => 'Please choose a profile image to upload.',
        ...
        try {
            ...
            DB::table('service_providers')
                ->where('id', $providerId)
                ->update([
                    'profile_image' => str_replace('\\', '/', $storedPath),
                    'updated_at' => now(),
                ]);
            ...
            Log::error('Provider profile image update failed', [
                'provider_id' => $providerId,
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);

            return back()->with('error', 'Profile image update failed: ' . $e->getMessage());
        }
    }
